...
describe("useDashboardSalesPeriod", () => {
  ...
  it("separa resultados al alternar entre preset y Personalizado", async () => {
    const weekRequest = { preset: "week" } as const;
    const customRequest = { preset: "custom", startDate: "2024-03-20", endDate: "2024-03-27" } as const;
    ...
    const { result, rerender } = renderHook(({ request }) => useDashboardSalesPeriod(request), {
      initialProps: { request: weekRequest },
